...
public class ImportDB {
    ...
    private void createTable() throws ClassNotFoundException, SQLException {
        ...
        try (Connection cnt = DriverManager.getConnection(cfg.getProperty("jdbc.url"),
                cfg.getProperty("jdbc.username"),
                cfg.getProperty("jdbc.password"))) {
            try (Statement statement = cnt.createStatement()) {
                statement.execute(String.format("CREATE TABLE IF NOT EXISTS users "
                                + "(%s, %s, %s);",
    ...
    private void check(String[] str) {
        if (str.length < 2
                || !str[0].matches("[\\w ]+")
                || !str[1].matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+$")) {
